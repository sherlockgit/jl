	...
	methods: {
        ...
        excle: function (event) {
            ...
            layer.confirm('确定要导出数据？', function(index){
                window.location =baseURL + "sys/invoiceinfo/getExcle?invoiceNo="+vm.q.invoiceNo+"&expressNo="+vm.q.expressNo+"&cneeName="+vm.q.cneeName+"&invoiceStauts="+vm.q.invoiceStauts+"&invoiceType="+vm.q.invoiceType+"&invoiceCategory="+vm.q.invoiceCategory+"&startTime="+vm.q.startTime+"&endTime="+vm.q.endTime;
                layer.close(index);
                vm.reload();
            });
